...
**A**. ネイティブプラットフォームでは、Dart は通常通りに 1 つのスレッドで実行されますが、Rust は`tokio`ランタイムを利用してコンピューターのすべてのコアを活用し、非同期タスクをそのランタイム内で効率的に実行します。これにより、リソースのより良い活用とパフォーマンスの向上が可能となります。Web 上では、Dart は引き続きメインスレッドで実行されますが、Rust は 1 つの Web ワーカー（スレッド）内で動作します。これは、Web ワーカーがメモリを共有しないという制約があるため、必要な制約です。しかし、Rust は依然としてその 1 つの専用スレッド内で並行操作を実行することができ、Rust の`Future`を JavaScript の`Promise`に変換することで実現されます。
