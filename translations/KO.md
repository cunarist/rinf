...
**Q**. 병렬 작업들은 내부적으로 어떻게 작동하나요?

**A**. 네이티브 플랫폼에서 Dart는 당연히 단일 스레드에서 실행되며, Rust는 `tokio` 런타임을 활용하여 컴퓨터의 모든 코어를 활용하고, 비동기 작업이 그 런타임 내에서 효율적으로 실행될 수 있도록 합니다. 이렇게 함으로써 리소스를 더 잘 활용하고 성능을 향상시킬 수 있습니다. 웹에서는 Dart가 메인 스레드에서 실행되며, Rust는 하나의 Web worker(스레드) 내에서 작동합니다. Web worker는 메모리를 공유하지 않기 때문에 이 제약이 불가피하지만, 여전히 Rust는 그 하나의 전용 스레드 내에서 동시 작업을 수행할 수 있습니다.
