...
**A**. 바이너리를 빌드하고 배포하기 전에, 웹 서버가 특정 값의 헤더를 응답에 포함시키도록 해야 합니다. `Cross-Origin-Resource-Policy`의 값으로 `same-origin`을, 그리고 `Cross-Origin-Embedder-Policy`를 `require-corp`로 설정하세요.
