...
Make sure to check the terminal output and fill in those necessary files after the generation process is complete.
...
`lib.rs` inside `./native/hub/src` is the entry point of your Rust logic while `main.dart` inside `./lib` is the entry point of your Dart logic.
