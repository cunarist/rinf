...
/// Marks the struct as a signal that can be used within other signals.
/// This does not enable sending signals directly,
/// but allows Dart to recognize the nested type.
#[proc_macro_derive(Signal)]
pub fn derive_signal(_input: TokenStream) -> TokenStream {
    TokenStream::new()
}

/// Marks the struct as a signal endpoint that contains a message from Dart.
...
/// Marks the struct as a signal endpoint that contains a message from Rust.
